...
CHIRPS_RAW_DIR = DATA_DIR / "public" / "raw" / "hti" / "chirps" / "daily"
CHIRPS_PROC_DIR = DATA_DIR / "public" / "processed" / "hti" / "chirps"
CHIRPS_BASE_URL = (
...
def calculate_raster_stats():
    ...
    for q in range(10, 91, 10):
        stats[f"q{q}"] = ds_clip["prcp"].quantile(q / 100, dim=["X", "Y"])
    stats = stats.drop_vars("quantile")
    df = stats.to_dataframe().drop(columns=["spatial_ref"]).reset_index()
    df["T"] = df["T"].dt.date
...
def load_chirps_daily():
    ds = xr.open_dataset(CHIRPS_PROC_DIR / "chirps-daily-2000-2023.nc")
...
def process_chirps_daily():
    ...
    filename = "chirps-daily-2000-2023.nc"
